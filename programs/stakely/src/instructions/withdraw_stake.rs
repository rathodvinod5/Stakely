...
pub fn withdraw_stake(ctx: Context<WithdrawStakeAmount>) -> Result<()> {
    ...
    // require!(pool.deactivating_stake_accounts.contains(&stake_account.key()), CustomErrors::InvalidStakeAccount); // uncomment this later
    ...
    // pool.deactivating_stake_accounts.retain(|pub_key| pub_key != &stake_account.key()); // uncomment this later
