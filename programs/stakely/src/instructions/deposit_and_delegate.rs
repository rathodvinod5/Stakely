...
pub fn deposit_and_delegate(ctx: Context<DepositAndDelegate>, stake_amount: u64) -> Result<()> {
    ...
    require!(
        stake_amount > 0 || (actual_amount > stake_amount + rent_exempt),
        CustomErrors::InsufficientBalance
    );

    // If the ATA does not exist, Anchor sees data_is_empty()
    ...
    //  calculate number lst's to mint
    let mut lst_to_mint = 0;
    if pool.total_staked == 0 && pool.total_lst_minted == 0 {
        lst_to_mint = (stake_amount as u128)
            .checked_mul((10 as u128).pow(pool.lst_decimals as u32))
            .unwrap();
    } else {
        lst_to_mint = (stake_amount as u128)
            .checked_mul(pool.total_lst_minted)
            .unwrap()
    ...
    let lst_amount = lst_to_mint
        .try_into()
        .map_err(|_| error!(CustomErrors::MathOverflow))?;

    let _ = token::mint_to(cpi_context, lst_amount);

    // upate stake_entry
    stake_entry.pool = pool.key();
    stake_entry.stake_account = stake_account.key();
    // stake_entry.validator_voter = ctx.accounts.validator_voter.key();
    stake_entry.deposited_lamports = stake_amount as u128;
    stake_entry.stake_status = StakeStatus::Active;
    stake_entry.index = pool.staked_count;

    // update pool params
    pool.staked_count = pool.staked_count.checked_add(1).unwrap();
    pool.total_staked = pool.total_staked.checked_add(stake_amount.into()).unwrap();
    pool.total_lst_minted = pool.total_lst_minted.checked_add(lst_to_mint).unwrap();
...
pub struct DepositAndDelegate<'info> {
    ...
    // #[account(mut)]
    // pub validator_voter: AccountInfo<'info>,
    #[account(
        mut,
        owner = crate::ID,
    ...
    #[account(
        ...
        seeds = [b"stake-entry", pool.key().as_ref(), user.key().as_ref()],
        bump
    )]
